...
class Community(models.Model):
    ...
    cover_image = models.ImageField(
        upload_to="community_covers/", blank=True, null=True
    )

    # Privacy & Access
    privacy = models.CharField(max_length=20, choices=PRIVACY_CHOICES, default="public")
    university = models.CharField(
        max_length=100, choices=Profile.UNIVERSITY_CHOICES, blank=True, null=True
    )
...
class Thread(models.Model):
    ...
    community = models.OneToOneField(
        Community, on_delete=models.CASCADE, related_name="chat_thread"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class EventRSVP(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="event_rsvps"
    )
