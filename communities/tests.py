...
class CommunityModelTests(TestCase):
    ...
    def test_auto_slug_counter_increment(self):
        ...
        community = Community.objects.create(
            name="Auto Generated Slug", description="Test", created_by=self.user
        )
    ...
    def test_unique_name_constraint(self):
        ...
        Community.objects.create(
            name="Unique Name", description="First", created_by=self.user
        )
        with self.assertRaises(IntegrityError):
            Community.objects.create(
                name="Unique Name", description="Duplicate", created_by=self.user
            )
    ...
    def test_community_ordering(self):
        ...
        comm1 = Community.objects.create(
            name="First", description="First", created_by=self.user
        )
        comm2 = Community.objects.create(
            name="Second", description="Second", created_by=self.user
        )
...
class CommunityMemberModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username="user1", email="user1@nyu.edu", password="testpass123"
        )
        self.user2 = User.objects.create_user(
            username="user2", email="user2@columbia.edu", password="testpass123"
        )
        self.community = Community.objects.create(
            name="Test Community", description="Test description", created_by=self.user1
        )

    def test_create_membership(self):
        """Test creating a community membership"""
        membership = CommunityMember.objects.create(
            community=self.community, user=self.user1, role="member", status="active"
        )
        self.assertEqual(membership.community, self.community)
        self.assertEqual(membership.user, self.user1)
        self.assertEqual(membership.role, "member")
        self.assertEqual(membership.status, "active")
        self.assertEqual(str(membership), "user1 - Test Community (member)")

    def test_default_member_role(self):
        """Test default role is 'member'"""
        membership = CommunityMember.objects.create(
            community=self.community, user=self.user1
        )
        self.assertEqual(membership.role, "member")
    ...
    def test_moderator_role(self):
        ...
        membership = CommunityMember.objects.create(
            community=self.community, user=self.user1, role="moderator"
        )
    ...
    def test_admin_role(self):
        ...
        membership = CommunityMember.objects.create(
            community=self.community, user=self.user1, role="admin"
        )
    ...
    def test_pending_status(self):
        ...
        self.assertEqual(
            membership.request_message, "I would like to join this community"
        )

    def test_banned_status(self):
        """Test banned membership status"""
        membership = CommunityMember.objects.create(
            community=self.community, user=self.user2, status="banned"
        )
        self.assertEqual(membership.status, "banned")

    def test_unique_together_constraint(self):
        """Test user can only have one membership per community"""
        CommunityMember.objects.create(community=self.community, user=self.user1)
        with self.assertRaises(IntegrityError):
            CommunityMember.objects.create(community=self.community, user=self.user1)
    ...
    def test_multiple_communities_same_user(self):
        ...
        community2 = Community.objects.create(
            name="Second Community", description="Second", created_by=self.user2
        )

        membership1 = CommunityMember.objects.create(  # noqa: F841
            community=self.community, user=self.user1
        )
        membership2 = CommunityMember.objects.create(  # noqa: F841
            community=community2, user=self.user1
        )
    ...
    def test_membership_ordering(self):
        ...
        mem1 = CommunityMember.objects.create(community=self.community, user=self.user1)
        mem2 = CommunityMember.objects.create(community=self.community, user=self.user2)
    ...
    def test_reviewed_by_field(self):
        ...
        membership = CommunityMember.objects.create(
            community=self.community, user=self.user2, status="pending"
        )
...
class CommunityViewTests(TestCase):
    ...
    def test_create_community_post_valid(self):
        ...
        membership = CommunityMember.objects.get(community=community, user=self.user1)
        self.assertEqual(membership.role, "admin")
        self.assertEqual(membership.status, "active")
    ...
    def test_join_public_community(self):
        ...
        membership = CommunityMember.objects.get(
            community=self.public_community, user=self.user2
        )
        self.assertEqual(membership.status, "active")
        self.assertEqual(membership.role, "member")
    ...
    def test_join_private_community_creates_request(self):
        ...
        membership = CommunityMember.objects.get(
            community=self.private_community, user=self.user2
        )
        self.assertEqual(membership.status, "pending")
    ...
    def test_join_community_already_member(self):
        ...
        CommunityMember.objects.create(
            community=self.public_community, user=self.user2, status="active"
        )
    ...
    def test_leave_community_last_admin_prevented(self):
        ...
        self.assertTrue(
            CommunityMember.objects.filter(
                community=self.public_community, user=self.user1
            ).exists()
        )

        messages = list(response.wsgi_request._messages)
        self.assertTrue(any("last admin" in str(m).lower() for m in messages))
    ...
    def test_promote_member_by_admin(self):
        ...
        response = self.client.post(
            reverse(
                ...
                args=[self.public_community.slug, self.user2.id],
            )
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_remove_member(self):
        ...
        # Verify membership deleted
        self.assertFalse(CommunityMember.objects.filter(id=member.id).exists())
    ...
    def test_join_requests_view(self):
        ...
        self.client.login(username="user1@nyu.edu", password="testpass123")
        response = self.client.get(
            reverse("communities:join_requests", args=[self.private_community.slug])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "user2")
    ...
    def test_approve_join_request(self):
        ...
        request_membership = CommunityMember.objects.create(
            community=self.private_community, user=self.user2, status="pending"
        )

        self.client.login(username="user1@nyu.edu", password="testpass123")
        response = self.client.post(
            reverse(
                "communities:approve_request",
                args=[self.private_community.slug, self.user2.id],
            )
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_non_moderator_cannot_manage_members(self):
        ...
        response = self.client.post(
            reverse(
                "communities:remove_member",
                args=[self.public_community.slug, self.user3.id],
            )
        )
        self.assertEqual(response.status_code, 403)
...
class PostModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.community = Community.objects.create(
            name="Test Community", description="Test description", created_by=self.user
        )
        CommunityMember.objects.create(
            community=self.community, user=self.user, role="member", status="active"
        )
    ...
    def test_post_ordering(self):
        ...
        post1 = Post.objects.create(
            community=self.community, author=self.user, content="First post"
        )
        post2 = Post.objects.create(
            community=self.community, author=self.user, content="Second post"
        )
...
class PostImageTests(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(
            community=self.community, author=self.user, content="Test post"
        )
...
class PostFileTests(TestCase):
    ...
    def test_create_post_file(self):
        ...
        post_file = PostFile.objects.create(
            post=self.post, file=file, file_name="test_file.pdf", file_size=17
        )
    ...
    def test_file_auto_name_and_size(self):
        ...
        post_file = PostFile.objects.create(post=self.post, file=file)
...
class CommentModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.post = Post.objects.create(
            community=self.community, author=self.user1, content="Test post"
        )

    def test_create_comment(self):
        """Test creating a comment on a post"""
        comment = Comment.objects.create(
            post=self.post, author=self.user2, content="This is a test comment"
        )
    ...
    def test_create_nested_comment(self):
        ...
        parent_comment = Comment.objects.create(
            post=self.post, author=self.user1, content="Parent comment"
        )
    ...
    def test_comment_ordering(self):
        ...
        comment1 = Comment.objects.create(
            post=self.post, author=self.user1, content="First comment"
        )
        comment2 = Comment.objects.create(
            post=self.post, author=self.user2, content="Second comment"
        )
    ...
    def test_comment_str(self):
        ...
        comment = Comment.objects.create(
            post=self.post, author=self.user2, content="Test comment"
        )
    ...
    def test_multiple_nested_levels(self):
        ...
        comment1 = Comment.objects.create(
            post=self.post, author=self.user1, content="Level 1"
        )
        comment2 = Comment.objects.create(
            post=self.post, author=self.user2, content="Level 2", parent=comment1
        )
        comment3 = Comment.objects.create(
            post=self.post, author=self.user1, content="Level 3", parent=comment2
        )
...
class PostViewTests(TestCase):
    ...
    def test_create_post_with_title_and_content(self):
        ...
        self.client.login(username="regular@nyu.edu", password="testpass123")
        data = {"title": "Test Post Title", "content": "This is the post content"}
        response = self.client.post(
            reverse("communities:create_post", args=[self.community.slug]), data
        )
    ...
    def test_create_post_without_title(self):
        ...
        self.client.login(username="regular@nyu.edu", password="testpass123")
        data = {"content": "Post without title"}
        response = self.client.post(
            reverse("communities:create_post", args=[self.community.slug]), data
        )
    ...
    def test_create_post_with_images(self):
        ...
        data = {"content": "Post with images", "images": [image1, image2]}
        response = self.client.post(
            reverse("communities:create_post", args=[self.community.slug]), data
        )
    ...
    def test_create_post_with_files(self):
        ...
        data = {"content": "Post with files", "files": [file1, file2]}
        response = self.client.post(
            reverse("communities:create_post", args=[self.community.slug]), data
        )
    ...
    def test_create_post_image_limit(self):
        ...
        data = {"content": "Post with many images", "images": images}
        response = self.client.post(
            reverse("communities:create_post", args=[self.community.slug]), data
        )
    ...
    def test_post_detail_view_non_member_forbidden(self):
        ...
        post = Post.objects.create(
            community=self.community, author=self.regular_user, content="Test content"
        )
    ...
    def test_post_detail_shows_images_and_files(self):
        ...
        post = Post.objects.create(
            community=self.community, author=self.regular_user, content="Test post"
        )
    ...
    def test_edit_post_as_author(self):
        ...
        response = self.client.post(
            reverse("communities:edit_post", args=[self.community.slug, post.id]), data
        )
    ...
    def test_edit_post_as_moderator(self):
        ...
        data = {"content": "Moderator edited content"}
        response = self.client.post(
            reverse("communities:edit_post", args=[self.community.slug, post.id]), data
        )
    ...
    def test_edit_post_non_author_non_moderator_forbidden(self):
        ...
        CommunityMember.objects.create(
            community=self.community, user=another_user, role="member", status="active"
        )
    ...
    def test_delete_post_as_author(self):
        ...
        post = Post.objects.create(
            community=self.community, author=self.regular_user, content="To be deleted"
        )
    ...
    def test_delete_post_non_author_non_moderator_forbidden(self):
        ...
        post = Post.objects.create(
            community=self.community, author=self.regular_user, content="Cannot delete"
        )
    ...
    def test_pin_post_as_moderator(self):
        ...
        post = Post.objects.create(
            community=self.community, author=self.regular_user, content="To be pinned"
        )
    ...
    def test_community_detail_shows_pinned_posts_first(self):
        ...
        post1 = Post.objects.create(  # noqa: F841
            community=self.community, author=self.regular_user, content="Regular post 1"
        )
        post2 = Post.objects.create(  # noqa: F841
            community=self.community, author=self.regular_user, content="Regular post 2"
        )
...
class CommentViewTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create memberships
        CommunityMember.objects.create(
            community=self.community, user=self.author, role="admin", status="active"
        )
        CommunityMember.objects.create(
            community=self.community,
            user=self.commenter,
    ...
    def test_create_comment(self):
        ...
        response = self.client.post(
            reverse(
                "communities:create_comment", args=[self.community.slug, self.post.id]
            ),
            data,
        )
    ...
    def test_create_reply_comment(self):
        ...
        parent_comment = Comment.objects.create(
            post=self.post, author=self.author, content="Parent comment"
        )

        self.client.login(username="commenter@nyu.edu", password="testpass123")
        data = {"content": "Reply to parent", "parent_id": parent_comment.id}
        response = self.client.post(
            reverse(
                "communities:create_comment", args=[self.community.slug, self.post.id]
            ),
    ...
    def test_edit_comment_as_author(self):
        ...
        comment = Comment.objects.create(
            post=self.post, author=self.commenter, content="Original comment"
        )
        ...
        response = self.client.post(
            reverse(
                ...
                args=[self.community.slug, self.post.id, comment.id],
            ),
            data,
        )
    ...
    def test_delete_comment_as_author(self):
        ...
        comment = Comment.objects.create(
            post=self.post, author=self.commenter, content="To be deleted"
        )

        self.client.login(username="commenter@nyu.edu", password="testpass123")
        response = self.client.post(
            reverse(
                "communities:delete_comment",
                args=[self.community.slug, self.post.id, comment.id],
            )
        )
    ...
    def test_delete_comment_as_moderator(self):
        ...
        comment = Comment.objects.create(
            post=self.post, author=self.commenter, content="To be deleted by moderator"
        )
    ...
    def test_delete_comment_non_author_non_moderator_forbidden(self):
        ...
        comment = Comment.objects.create(
            post=self.post, author=self.commenter, content="Cannot be deleted"
        )
    ...
    def test_post_detail_shows_top_level_comments(self):
        ...
        comment1 = Comment.objects.create(
            post=self.post, author=self.author, content="Top level comment 1"
        )
        comment2 = Comment.objects.create(  # noqa: F841
            post=self.post, author=self.commenter, content="Top level comment 2"
        )
        # Create reply (should not appear in top-level list)
        Comment.objects.create(
            post=self.post, author=self.author, content="Reply", parent=comment1
        )
    ...
    def test_deleting_parent_comment_deletes_replies(self):
        ...
        parent = Comment.objects.create(
            post=self.post, author=self.author, content="Parent"
        )
        reply1 = Comment.objects.create(
            post=self.post, author=self.commenter, content="Reply 1", parent=parent
        )
        reply2 = Comment.objects.create(
            post=self.post, author=self.commenter, content="Reply 2", parent=parent
        )
        ...
        self.client.post(
            reverse(
                "communities:delete_comment",
                args=[self.community.slug, self.post.id, parent.id],
            )
        )
...
class PostFormTests(TestCase):
    ...
    def test_post_form_title_max_length(self):
        ...
        form = PostForm(data={"title": long_title, "content": "Content"})
        self.assertFalse(form.is_valid())
    ...
    def test_post_form_content_max_length(self):
        ...
        form = PostForm(data={"title": "Title", "content": long_content})
        self.assertFalse(form.is_valid())
...
class PermissionHelperTests(TestCase):
    ...
    def setUp(self):
        ...
        self.admin = User.objects.create_user(
            username="admin", email="admin@nyu.edu", password="testpass123"
        )
        self.moderator = User.objects.create_user(
            username="moderator", email="moderator@nyu.edu", password="testpass123"
        )
        self.member = User.objects.create_user(
            username="member", email="member@nyu.edu", password="testpass123"
        )
        ...
        self.outsider = User.objects.create_user(
            username="outsider", email="outsider@columbia.edu", password="testpass123"
        )

        self.community = Community.objects.create(
            name="Test Community", description="Test", created_by=self.admin
        )

        CommunityMember.objects.create(
            community=self.community, user=self.admin, role="admin", status="active"
        )
        CommunityMember.objects.create(
            community=self.community,
            user=self.moderator,
        ...
        CommunityMember.objects.create(
            community=self.community, user=self.member, role="member", status="active"
        )
...
class UtilityFunctionTests(TestCase):
    ...
    def test_validate_university_access_profile_match(self):
        ...
        self.assertTrue(validate_university_access(self.nyu_user, self.nyu_community))
    ...
    def test_validate_university_access_email_domain_match(self):
        ...
        self.assertTrue(
            validate_university_access(self.barnard_user, self.columbia_community)
        )
    ...
    def test_validate_university_access_no_match(self):
        ...
        self.assertFalse(
            validate_university_access(self.nyu_user, self.columbia_community)
        )
    ...
    def test_validate_university_access_public_community(self):
        ...
        self.assertTrue(
            validate_university_access(self.columbia_user, public_community)
        )
    ...
    def test_get_user_university_from_email(self):
        ...
        self.assertEqual(get_user_university_from_email("student@nyu.edu"), "NYU")
        self.assertEqual(
            get_user_university_from_email("student@columbia.edu"), "Columbia"
        )
        self.assertEqual(
            get_user_university_from_email("student@barnard.edu"), "Columbia"
        )
        self.assertIsNone(get_user_university_from_email("student@unknown.edu"))
...
class IntegrationTests(TestCase):
    ...
    def test_complete_post_workflow(self):
        ...
        response = self.client.post(reverse("communities:join", args=[community.slug]))
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.post(
            reverse("communities:create_comment", args=[community.slug, post.id]),
            {"content": "Thanks!", "parent_id": comment.id},
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_post_edit_marks_as_edited(self):
        ...
        self.client.post(
            reverse("communities:create"),
            {"name": "Test Community", "description": "Test", "privacy": "public"},
        )
...
class ThreadModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="test@nyu.edu", username="testuser", password="testpass123"
        )
...
class ChatMessageModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            email="user1@nyu.edu", username="user1", password="testpass123"
        )
        Profile.objects.create(user=self.user1, university="NYU")

        self.user2 = User.objects.create_user(
            email="user2@nyu.edu", username="user2", password="testpass123"
        )
        Profile.objects.create(user=self.user2, university="NYU")
    ...
    def test_chat_message_str(self):
        ...
        message = ChatMessage.objects.create(
            thread=self.thread, sender=self.user1, content="Test message"
        )
    ...
    def test_chat_message_ordering(self):
        ...
        msg1 = ChatMessage.objects.create(
            thread=self.thread, sender=self.user1, content="First"
        )
        msg2 = ChatMessage.objects.create(
            thread=self.thread, sender=self.user2, content="Second"
        )
    ...
    def test_chat_message_is_edited_default(self):
        ...
        message = ChatMessage.objects.create(
            thread=self.thread, sender=self.user1, content="Test"
        )
        self.assertFalse(message.is_edited)
    ...
    def test_multiple_messages_same_thread(self):
        ...
        ChatMessage.objects.create(
            thread=self.thread, sender=self.user1, content="Message 1"
        )
        ChatMessage.objects.create(
            thread=self.thread, sender=self.user2, content="Message 2"
        )
        ChatMessage.objects.create(
            thread=self.thread, sender=self.user1, content="Message 3"
        )
    ...
    def test_delete_thread_deletes_messages(self):
        ...
        self.assertEqual(ChatMessage.objects.filter(thread_id=thread_id).count(), 0)
...
class EventRSVPModelTests(TestCase):
    ...
    def test_rsvp_creation(self):
        ...
        rsvp = EventRSVP.objects.create(
            event=self.event, user=self.user1, status="going"
        )
    ...
    def test_rsvp_str(self):
        ...
        rsvp = EventRSVP.objects.create(
            event=self.event, user=self.user1, status="interested"
        )
    ...
    def test_rsvp_unique_constraint(self):
        ...
        EventRSVP.objects.create(event=self.event, user=self.user1, status="going")
    ...
    def test_multiple_rsvps_different_users(self):
        ...
        rsvp1 = EventRSVP.objects.create(  # noqa: F841
            event=self.event, user=self.user1, status="going"
        )
        rsvp2 = EventRSVP.objects.create(  # noqa: F841
            event=self.event, user=self.user2, status="interested"
        )
