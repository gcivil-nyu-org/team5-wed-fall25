...
urlpatterns = [
    ...
    path(
        "<slug:slug>/requests/<int:user_id>/approve/",
        views.approve_request,
        name="approve_request",
    ),
    path(
        "<slug:slug>/requests/<int:user_id>/reject/",
    ...
    path(
        "<slug:slug>/posts/<int:post_id>/delete/", views.delete_post, name="delete_post"
    ),
    path(
        "<slug:slug>/posts/<int:post_id>/pin/",
        views.toggle_pin_post,
        name="toggle_pin_post",
    ),
    # Comments (Phase 2)
    path(
        "<slug:slug>/posts/<int:post_id>/comments/create/",
        views.create_comment,
        name="create_comment",
    ),
    path(
        "<slug:slug>/posts/<int:post_id>/comments/<int:comment_id>/edit/",
        views.edit_comment,
        name="edit_comment",
    ),
    path(
    ...
    path(
        "<slug:slug>/events/<int:event_id>/edit/", views.edit_event, name="edit_event"
    ),
    path(
        "<slug:slug>/events/<int:event_id>/delete/",
        views.delete_event,
        name="delete_event",
    ),
    path(
        "<slug:slug>/events/<int:event_id>/rsvp/", views.rsvp_event, name="rsvp_event"
    ),
    path(
        "<slug:slug>/events/<int:event_id>/cancel/",
        views.cancel_event,
        name="cancel_event",
    ),
]
