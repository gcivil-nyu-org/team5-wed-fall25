/**
 * Listing Location Picker - Interactive Map for Selecting Listing Location
 *
 * This script initializes a Mapbox map that allows users to:
 * 1. Click on the map to set listing location
...
document.addEventListener("DOMContentLoaded", function () {
  const mapDiv = document.getElementById("listing-location-map");
  ...
  // Check if editing existing listing with coordinates
  ...
  try {
    ...
    const map = new mapboxgl.Map({
      container: "listing-location-map",
    ...
    const geocoder = new MapboxGeocoder({
      ...
      placeholder: "Search for listing location...",
      proximity: {
        longitude: -73.935242,
        latitude: 40.73061,
    ...
    function updateLocation(lngLat) {
      ...
      if (marker) {
        ...
        marker = new mapboxgl.Marker({
          draggable: true,
          color: "#3b82f6", // Blue color for listings
        })
          .setLngLat([lng, lat])
          .addTo(map);
    ...
    console.log("Listing location picker initialized successfully");
  } catch (error) {
    console.error("Error initializing listing location picker:", error);
