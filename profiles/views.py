...
def my_favorites(request):
    ...
    favorites = Favorite.objects.filter(user=request.user).select_related('favorite_profile')
...
def respond_to_request(request, request_id):
    ...
    if action == 'accept':
        ...
        conn_request.save()
        messages.success(request, f"Connected with {conn_request.from_user.first_name}!")
    elif action == 'reject':
